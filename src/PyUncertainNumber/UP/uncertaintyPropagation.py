...
def aleatory_propagation(vars:list = None,
    ...
    match method:
        ...
        case ("monte_carlo" | "latin_hypercube"): 
            ...
            results= sampling_aleatory_method(vars, fun, n, method=method.lower(), results=results, save_raw_data= save_raw_data)    
...
def epistemic_propagation(vars,
    ...
    match method:
        ...
        case ("endpoint" | "endpoints" | "vertex"):
            results = endpoints_method(vars, fun, results, save_raw_data)  # Pass save_raw_data directly
            return process_results(results) 
        
        case ("endpoints_monotonic" ):       
            results = endpoints_extremepoint_method(vars, fun,  results,save_raw_data)  # Pass save_raw_data directly
            return process_results(results) 

        case ("subinterval" | "subinterval_reconstitution"):
            if n is None:
                raise ValueError("n (number of subintervals) is required for subinterval methods.")
            results = subinterval_method(vars, fun, n,  results, save_raw_data)  # Pass save_raw_data directly
            return process_results(results)
        
        case ("monte_carlo" |  "latin_hypercube"): 
            if n is None:
                raise ValueError("n (number of samples) is required for sampling methods.")            
            results= sampling_method(vars, fun, n, method=method.lower(), endpoints=False,  results=results, save_raw_data= save_raw_data)    
            return process_results(results)
                
        case ("monte_carlo_endpoints" ): 
            if n is None:
                raise ValueError("n (number of samples) is required for sampling methods.")           
            results= sampling_method(vars, fun, n, method= "monte_carlo", endpoints=True,  results=results, save_raw_data= save_raw_data)    
            return process_results(results)

        case ("latin_hypercube_endpoints" ): 
            if n is None:
                raise ValueError("n (number of samples) is required for sampling methods.")           
            results= sampling_method(vars, fun, n, method="latin_hypercube", endpoints=True,  results=results, save_raw_data= save_raw_data)    
            return process_results(results)   

        case ("cauchy" |  "endpoint_cauchy"|  "endpoints_cauchy"): 
            if n is None:
                raise ValueError("n (number of samples) is required for sampling methods.")            
            results= cauchydeviates_method(vars,fun, n, results, save_raw_data)
            return process_results(results)    
        ...
        case ("local_optimization" | "local_optimisation"|"local optimisation"|"local optimization") :
            ...
            results = local_optimisation_method(vars, fun, x0, results=results,
                                             tol_loc = tol_loc, 
        ...
        case ("genetic_optimisation" | "genetic_optimization"| "genetic optimization"|"genetic optimisation"):
            ...
            results = genetic_optimisation_method(vars, fun, results, pop_size, n_gen, tol, n_gen_last, 
                                               algorithm_type)                          
            return process_results(results) 
