...
def betabinomial2(size, v, w): return (
    scipy.stats.binom.rvs(size, beta(v, w), size=Params.many))


def betabinomial(size, v, w): return (
    scipy.stats.betabinom.rvs(size, v, w, size=Params.many))


def binomial(size, p): return (
    scipy.stats.binom.rvs(size, p, size=Params.many))


def chisquared(v): return (scipy.stats.chi2.rvs(v, size=Params.many))


def delta(a): return (np.repeat(a, Params.many))
...
def gumbel(loc, scale): return (
    scipy.stats.gumbel_r.rvs(loc, scale, size=Params.many))
...
def logistic(loc, scale): return (
    scipy.stats.logistic.rvs(loc, scale, size=Params.many))
...
def lognormal2(mlog, slog): return (
    scipy.stats.lognorm.rvs(s=slog, scale=np.exp(mlog), size=Params.many))
...
def loguniform_solve(m, v):
    def loguniform_f(a, m, v): return (
        a*m*np.exp(2*(v/(m**2)+1)) + np.exp(2*a/m)*(a*m - 2*((m**2) + v)))
...
def loguniform(min=None, max=None, minlog=None, maxlog=None, mean=None, std=None):
    ...
    if (max is None) and (not (maxlog is None)):
        max = np.exp(maxlog)
    if (max is None) and (not (mean is None)) and (not (std is None)) and (not (min is None)):
...
def negativebinomial(size, prob): return (
    scipy.stats.nbinom.rvs(size, prob, size=Params.many))


def normal(m, s): return (scipy.stats.norm.rvs(m, s, size=Params.many))


def pareto(mode, c): return (
    scipy.stats.pareto.rvs(c, scale=mode, size=Params.many))


def poisson(m): return (scipy.stats.poisson.rvs(m, size=Params.many))


def powerfunction(b, c): return (
    scipy.stats.powerlaw.rvs(c, scale=b, size=Params.many))

# parameterisation of rayleigh differs from that in pba.r


def rayleigh(loc, scale): return (
    scipy.stats.rayleigh.rvs(loc, scale, size=Params.many))
...
def uniform(a, b): return (scipy.stats.uniform.rvs(
    a, b-a, size=Params.many))  # who parameterizes like this?!?!


def triangular(min, mode, max): return (np.random.triangular(
    min, mode, max, size=Params.many))  # cheating: uses random rather than scipy.stats


def histogram(x): return (
    x[(np.trunc(scipy.stats.uniform.rvs(size=Params.many)*len(x))).astype(int)])
