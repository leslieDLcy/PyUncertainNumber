...
def CBnormal_mu_interval(x, style='analytical', size=Params.many):
    """
    args:
        x: (array-like) the sample data or x will be a list of Marco's intervals
        style: (str) the style of the output CDF, either 'analytical' or 'samples'
        size: (int) the discritisation size. 
            meaning the no of ppf in analytical style and the no of MC samples in samples style
    ...
    match style:
        case 'analytical':
                ...
                xm = np.mean(x)
                s = np.std(x)
                # parametric PBOX constructor
                rv = t(n-1, loc=xm, scale = s/np.sqrt(n))
                x_support = np.linspace(rv.ppf(0.001), rv.ppf(0.999), size)
                # export to CBox at last step
                return x_support, rv.cdf(x_support)
        case 'samples':
            n = len(x)
            return(np.mean(x) + np.std(x) * student(n - 1) / np.sqrt(n)) # pop or sample std?
