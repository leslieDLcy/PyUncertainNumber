...
def BB_results(df_OUTPUT_INPUT, res_path):
    # disseminates the results of epistemic uncertainty methods.
    # creates a file with the raw results.
    # creates a file to store the NA values
    # Estimates the min-max values for each output QoI
    # Creates a file to store the OUTPUT_INPUT corresponidng to each min-max QoI values
    # retuns a data.frame with min-max for each QoI

    # header
    header = df_OUTPUT_INPUT.columns.values.tolist()

    # Return .csv with raw data only if asked ###
    create_csv(res_path, "Raw_data.csv", df_OUTPUT_INPUT, header)

    # if NA output create a log file with the input which caused these values.
    df_NA_unique = NA_values_with_input(df_OUTPUT_INPUT, header, res_path)
    ...
    create_csv(res_path, "min_max_input.csv", df_y_dat_all, df_y_dat_all.columns)
