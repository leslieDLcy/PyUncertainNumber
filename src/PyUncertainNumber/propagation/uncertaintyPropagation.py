...
from pyuncertainnumber.characterisation.uncertainNumber import UncertainNumber, Distribution
...
def aleatory_propagation(vars: list = None,
    ...
    def process_alea_results(results):
        ...
        if save_raw_data == "yes":
            ...
            save_results(results.raw_data, method=method,
                         res_path=res_path, fun=fun)
    ...
    match method:
        ...
        case ("monte_carlo" | "latin_hypercube"):
            ...
            results = sampling_aleatory_method(
                vars, fun,  results, n_sam, method=method.lower(), save_raw_data=save_raw_data)
  ...
  """
        if results.raw_data['bounds'] is None or results.raw_data['bounds'].size == 0:
            results.un = UncertainNumber(
                essence="interval", bounds=None, **kwargs)
        else:
            if results.raw_data['bounds'].ndim == 2:  # 2D array
                results.un = [UncertainNumber(
                    essence="interval", bounds=bound, **kwargs) for bound in results.raw_data['bounds']]
            # 1D array
            elif results.raw_data['bounds'].ndim == 1 and len(results.raw_data['bounds']) == 2:
                results.un = UncertainNumber(
    ...
    match method:
        ...
        case ("monte_carlo" | "latin_hypercube"):
            if n_sam is None:
                raise ValueError(
                    "n (number of samples) is required for sampling methods.")
            results = sampling_method(x, fun, results, n_sam,  method=method.lower(
            ), endpoints=False, save_raw_data=save_raw_data)
            return process_results(results)
        ...
        case ("monte_carlo_endpoints"):
            ...
            results = sampling_method(
                x, fun, results, n_sam,  method="monte_carlo", endpoints=True,  save_raw_data=save_raw_data)
            return process_results(results)
        ...
        case ("latin_hypercube_endpoints"):
            if n_sam is None:
                raise ValueError(
                    "n (number of samples) is required for sampling methods.")
            results = sampling_method(
                x, fun, results, n_sam, method="latin_hypercube", endpoints=True,  save_raw_data=save_raw_data)
            return process_results(results)
        ...
        case ("cauchy" | "endpoint_cauchy" | "endpoints_cauchy"):
            ...
            results = cauchydeviates_method(
                x, fun, results, n_sam, save_raw_data)
            return process_results(results)
...
#     # example
    ...
    y = UncertainNumber(name='beam width', symbol='y',
                        units='m', essence='interval', bounds=[0.145, 0.155])
    L = UncertainNumber(name='beam length', symbol='L',
                        units='m', essence='interval', bounds=[9.95, 10.05])
    I = UncertainNumber(name='moment of inertia', symbol='I', units='m',
                        essence='interval', bounds=[0.0003861591, 0.0005213425])
    F = UncertainNumber(name='vertical force', symbol='F',
                        units='kN', essence='interval', bounds=[11, 37])
    E = UncertainNumber(name='elastic modulus', symbol='E',
                        units='GPa', essence='interval', bounds=[200, 220])
