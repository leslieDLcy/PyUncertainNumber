...
def sampling_method(x: np.ndarray, f: Callable,
    ...
    print(
        f"Total number of input combinations for the {method} method: {n_sam}")
    ...
    if f is not None:  # Only evaluate if f is provided
        ...
        min_indices = np.array([np.where(all_output[:, i] == lower_bound[i])[
                               0] for i in range(num_outputs)])
        max_indices = np.array([np.where(all_output[:, i] == upper_bound[i])[
                               0] for i in range(num_outputs)])
