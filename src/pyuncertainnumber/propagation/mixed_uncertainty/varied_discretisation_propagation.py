...
def varied_discretisation_propagation_method(x: list, f:Callable = None,  
    ...
    notes:
        ...
        -   If 'n_disc' is an np.array, the code slices each number to the maximum number of 'n_disc'.
        -   The function handles different types of uncertain numbers (distributions and p-boxes for this version) and discretizes 
            them with the same number of n_disc.To ensure conservative results, the function employs an outward-directed discretization 
            approach when discretizing probability distributions and pboxes.For distributions that extend to infinity (e.g., normal distribution), 
            the discretization process incorporates cut-off points defined by the tOp (upper) and bOt (lower) parameters to bound the distribution.
    ...
    match method:
        ...
        case _: 
            raise ValueError(
                     "Invalid UP method! Invalid UP method! Only 'varied_discretisation_extremepoints' for the 'method' parameter is available.")
