...
class Interval:
    ...
    def __init__(
        ...
        self._lo = asarray(lo, dtype=float)
        self._hi = asarray(hi, dtype=float)  # check lo and hi have same shape
        self._shape = self._lo.shape
...
class LightInterval(Interval):
    def __init__(self, lo, hi):
        # Directly assign values without checks
        super().__init__(lo, hi, run_setup=False)

    def __repr__(self):
        return f"Lighter [{self._lo},{self._hi}]"
