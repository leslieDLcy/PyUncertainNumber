...
A conspicuous problem in probability distribution elicition, for example in probabilistic modelling analysis, is that the specification is typically precise, despite hardly justified by empirical informtion in many cases. Epistemic Uncertainy remains on the shape, parameters, and dependencies of the distributions. [Probability box](https://en.wikipedia.org/wiki/Probability_box) (abbreviated as `p-box`) essentially represents bounds on the cumulative distribution function of the underlying random variable. Let $[\overline{F}, \underline{F}]$ denotes the **set** of all nondecreasing functions from the reals into $[0,1]$ such that $\underline{F} \le F \le \overline{F}$. This means that, $[\overline{F}, \underline{F}]$ denotes a p-box for a random varaible $X$ whose c.d.f $F$ is unknown except that it is within the "box" circumscribed by the lower ($\underline{F}$) and upper bound ($\overline{F}$).
...
![pbox illustration](../../assets/pbox_illustration.png)
<!-- a plot -->
